...
from day1.pptdatabase import PPTDataBase
...
# chart1 左上方图
x, y, cx, cy = Inches(0.5), Inches(1), Inches(4), Inches(5)  # 按英尺标准指定x，y值
...
rows = pdb.GetValues(sql)
sztitle = rows[0]['title']
shtitle = rows[1]['title']
szamount = rows[0]['amount']
shamount = rows[1]['amount']
#坐标最大值计算
maxamount = 0
if szamount > shamount:
    maxamount = (szamount * 1.5)
else:
    maxamount = (shamount * 1.5)
titletxet = '沪深公司数量'
chart_data.categories = [u'%s'%(sztitle), u'%s'%(shtitle)]  # 图表加入两栏
chart_data.add_series(u'%s'%(titletxet), (szamount, shamount))  # 在两栏分别填入数据
...
new_paragraph.text = u'%s \n 沪：%s \n 深：%s'%(titletxet,shamount,szamount)  # 新标题
...
value_axis.maximum_scale = maxamount  # 纵坐标最大值
...
#获取数据填充
pdb2 = PPTDataBase()
sql2 = "select leve,levenum,leveratio/100 as leveratio from leveratiotb"
rows2 = pdb2.GetValues(sql2)
...
va = rows2[0]['leveratio']
vb = rows2[1]['leveratio']
vc = rows2[2]['leveratio']
vd = rows2[3]['leveratio']

title2 = '市值层级分布占比'
# chart 2 左下方图
x, y, cx, cy = Inches(5.5), Inches(1), Inches(4), Inches(5)  # 按英尺标准指定x，y值
chart_data = ChartData()
chart_data.categories = [a, b, c, d]
chart_data.add_series(u'%s' %(title2), (va, vb, vc, vd))
#chart_data.add_series('', (va, vb, vc, vd))
...
data_labels.number_format = '0.00%'  # 数值显示格式
